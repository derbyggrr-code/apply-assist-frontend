    ...
    <div style={styles.app}>
      ...
      <div className="aa-shell" style={styles.shell}>
        <div className="aa-sidebar" style={styles.sidebar}>
          ...
          <NavItem icon={FileText} label="Profile" active={tab === "profile"} onClick={() => setTab("profile")} />
          <NavItem icon={LayoutGrid} label="Matches" active={tab === "matches"} onClick={() => setTab("matches")} />
          <NavItem icon={ListChecks} label={`Tracker${trackerEntries.length ? ` (${trackerEntries.length})` : ""}`} active={tab === "tracker"} onClick={() => setTab("tracker")} />
          <NavItem icon={TrendingUp} label="Digest" active={tab === "digest"} onClick={() => setTab("digest")} />
        </div>
        ...
        <div className="aa-main" style={styles.main}>
          ...
          {ready && tab === "profile" && (
            <Section title="Your profile" subtitle="Saved to your backend database — resume text and preferences persist across sessions and devices.">
                    ...
                    <div style={styles.parsedGroup}>
                      ...
                      {parsedProfile.experience.map((e, i) => (
                        <div key={i} style={styles.expItem}>
                          ...
                          <div style={{ fontSize: 12, color: "#8B9199", marginBottom: 4 }}>{e.duration}</div>
                          <ul style={{ margin: 0, paddingLeft: 16 }}>
                            {(e.highlights || []).map((h, j) => <li key={j} style={{ fontSize: 12.5, color: "#A8AEB6", lineHeight: 1.5 }}>{h}</li>)}
                          </ul>
                        </div>
                      ))}
                    </div>
                  ...
                  {(parsedProfile.education || []).length > 0 && (
                    <div style={styles.parsedGroup}>
                      ...
                      {parsedProfile.education.map((ed, i) => (
                        <div key={i} style={{ fontSize: 12.5, color: "#A8AEB6" }}>{ed.degree} — {ed.institution} ({ed.year})</div>
                      ))}
                    </div>
                  )}
                </div>
          ...
          {ready && tab === "matches" && (
            <Section title="Matched openings" subtitle="Live listings pulled from Adzuna, scored against your resume.">
                  ...
                  return (
                    <div key={job.id} className="aa-job" tabIndex={0} style={{ ...styles.jobCard, borderColor: activeJob?.id === job.id ? "#3E7C6B" : "#2C333B" }}>
                      ...
                      {entry?.tailored && activeJob?.id === job.id && (
                        <TailoredView tailored={entry.tailored} onCopy={handleCopy} copied={copied} idPrefix={job.id} job={job} />
                      )}
                    </div>
                  );
                })}
                {scoredJobs.length === 0 && (
                  <p style={{ color: "#7C8590", fontSize: 14 }}>No jobs yet — click "Refresh jobs" to pull listings from Adzuna.</p>
                )}
              </div>
            </Section>
          )}
          ...
          {ready && tab === "tracker" && (
            <Section title="Application tracker" subtitle="Everything you've tailored or logged, in one place.">
              ...
              {trackerEntries.length === 0 && (
                <p style={{ color: "#7C8590", fontSize: 14 }}>No applications yet — generate a tailored application from the Matches tab to add one here.</p>
              )}
              ...
              <div style={styles.jobList}>
                {trackerEntries.map(([jobId, entry]) => (
                  <div key={jobId} style={styles.jobCard}>
                    ...
                    {entry.tailored && <TailoredView tailored={entry.tailored} onCopy={handleCopy} copied={copied} idPrefix={jobId} job={entry.job} />}
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            {entry.interviewPrep.questions.map((q, i) => (
                              <div key={i}>
                                ...
                                <div style={{ fontSize: 12.5, color: "#8B9199", marginTop: 2 }}>{q.tip}</div>
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          {ready && tab === "digest" && (
            <Section title="Weekly digest" subtitle="Computed live from your tracker. A copy is also emailed automatically every week by the backend scheduler.">
...
function NavItem({ icon: Icon, label, active, onClick }) {
  return (
    <div className="aa-nav-item" onClick={onClick} style={{ ...styles.navItem, color: active ? "#E8E6E1" : "#7C8590", borderColor: active ? "#3E7C6B" : "transparent" }}>
...
function TailoredView({ tailored, onCopy, copied, idPrefix, job }) {
  return (
    <div style={styles.resultGrid}>
      ...
      <div style={styles.resultCard}>
        <div style={styles.resultHeader}>
          ...
          <CopyBtn onCopy={() => onCopy(tailored.coverLetter, `${idPrefix}-cover`)} copied={copied === `${idPrefix}-cover`} />
        </div>
...
const styles = {
  app: { fontFamily: "Inter, -apple-system, 'Segoe UI', sans-serif", background: "#15191E", color: "#E8E6E1", minHeight: "100vh" },
  shell: { display: "flex", minHeight: "100vh" },
  sidebar: { width: 220, borderRight: "1px solid #2C333B", padding: "28px 20px", display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 },
  ...
  subtitle: { color: "#8B9199", fontSize: 14, marginTop: 8, lineHeight: 1.5 },
  blockLabel: { display: "block", fontSize: 13, color: "#8B9199", marginBottom: 8 },
  textarea: { width: "100%", minHeight: 180, background: "#1D2229", border: "1px solid #2C333B", borderRadius: 6, color: "#E8E6E1", padding: 16, fontSize: 14.5, lineHeight: 1.6, fontFamily: "inherit", resize: "vertical", boxSizing: "border-box" },
  footerRow: { display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14 },
  hint: { fontSize: 13, color: "#7C8590" },
  primaryBtn: { background: "#3E7C6B", color: "#EAF3F0", border: "none", borderRadius: 6, padding: "10px 16px", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontFamily: "inherit" },
  smallPrimaryBtn: { background: "#3E7C6B", color: "#EAF3F0", border: "none", borderRadius: 5, padding: "6px 12px", fontSize: 12.5, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, fontFamily: "inherit" },
  errorBox: { color: "#D97757", fontSize: 13, padding: "10px 0" },
  parsedBox: { marginTop: 20, display: "flex", flexDirection: "column", gap: 18 },
  parsedGroup: {},
  parsedLabel: { fontSize: 12.5, color: "#8B9199", marginBottom: 8, textTransform: "none" },
  chipRow: { display: "flex", flexWrap: "wrap", gap: 6 },
  chip: { background: "#1D2229", border: "1px solid #2C333B", borderRadius: 20, padding: "3px 10px", fontSize: 12 },
  expItem: { marginBottom: 12 },
  divider: { height: 1, background: "#2C333B", margin: "28px 0" },
  formGrid: { display: "flex", flexDirection: "column", gap: 16, maxWidth: 420 },
  label: { display: "flex", flexDirection: "column", gap: 6, fontSize: 13, color: "#8B9199" },
  input: { background: "#1D2229", border: "1px solid #2C333B", borderRadius: 6, color: "#E8E6E1", padding: "9px 12px", fontSize: 14, fontFamily: "inherit" },
  jobList: { display: "flex", flexDirection: "column", gap: 14 },
  jobCard: { border: "1px solid #2C333B", borderRadius: 8, padding: "16px 18px", background: "#1A1F25" },
  jobTop: { display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 },
  jobRole: { fontSize: 16, fontWeight: 600 },
  jobMeta: { fontSize: 13, color: "#8B9199", marginTop: 3 },
  scoreBadge: { background: "rgba(199,154,75,0.15)", color: "#C79A4B", fontSize: 13, fontWeight: 600, padding: "3px 10px", borderRadius: 20, flexShrink: 0 },
  jobDesc: { fontSize: 13.5, color: "#A8AEB6", marginTop: 10, lineHeight: 1.55 },
  jobActions: { display: "flex", alignItems: "center", gap: 12, marginTop: 12 },
  statusTag: { fontSize: 12.5, fontWeight: 600 },
  statusSelect: { background: "#1D2229", border: "1px solid #2C333B", borderRadius: 5, color: "#E8E6E1", padding: "5px 8px", fontSize: 12.5, fontFamily: "inherit" },
  resultGrid: { display: "flex", flexDirection: "column", gap: 14, marginTop: 14 },
  resultCard: { background: "#15191E", border: "1px solid #2C333B", borderRadius: 8, padding: 16 },
  ...
  copyBtn: { background: "none", border: "1px solid #2C333B", color: "#8B9199", borderRadius: 5, padding: "4px 9px", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 },
  bulletList: { margin: 0, paddingLeft: 18, display: "flex", flexDirection: "column", gap: 7 },
  bulletItem: { fontSize: 13.5, lineHeight: 1.5, color: "#D4D6D9" },
  coverLetter: { fontSize: 13.5, lineHeight: 1.65, color: "#D4D6D9", whiteSpace: "pre-wrap", margin: 0 },
  loadingBox: { display: "flex", alignItems: "center", gap: 10, color: "#8B9199", padding: "24px 0" },
  statRow: { display: "flex", gap: 18, marginBottom: 22, flexWrap: "wrap" },
  statChip: { fontSize: 13, color: "#8B9199" },
  digestBox: {
    background: "#1A1F25", border: "1px solid #2C333B", borderRadius: 8, padding: 20,
    fontSize: 13, lineHeight: 1.7, color: "#D4D6D9", whiteSpace: "pre-wrap",
